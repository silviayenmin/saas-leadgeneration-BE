...
class TwitterAdapter:
    ...
    def search(self, keyword: str, timeframe: str = "qdr:m3", match_type: str = "partial", location: str = None, industry: str = None, api_key: str = None, limit: int = 10, raw_keyword: str = None) -> list:
        ...
        try:
            ...
            print(f"[TwitterAdapter] Trying Apify search for keyword: {search_term}")
            apify_results = scrape_apify_leads("twitter", search_term, limit=limit)
            if apify_results is not None:
                return apify_results
            print("[TwitterAdapter] Apify returned None or is not available. Falling back to Serper.")
        except Exception as ae:
            print(f"[TwitterAdapter] Apify search failed: {ae}. Falling back to Serper.")
        ...
        print(f"[TwitterAdapter] Searching query via Serper fallback: {query}")
        try:
            serper_results = search_leads(query, tbs=timeframe, api_key=api_key, num=limit)
            return [r for r in serper_results if "twitter.com" in (r.get("link") or "").lower() or "x.com" in (r.get("link") or "").lower()]
        except Exception as e:
            print(f"[TwitterAdapter] Serper Fallback Error: {e}")
            return []

...
class LinkedInAdapter:
    ...
    def search(self, keyword: str, timeframe: str = "qdr:m3", match_type: str = "partial", location: str = None, industry: str = None, api_key: str = None, limit: int = 10, raw_keyword: str = None) -> list:
        ...
        try:
            ...
            print(f"[LinkedInAdapter] Trying Apify search for keyword: {search_term}")
            apify_results = scrape_apify_leads("linkedin", search_term, limit=limit)
            if apify_results is not None:
                return apify_results
            print("[LinkedInAdapter] Apify returned None or is not available. Falling back to Serper.")
        except Exception as ae:
            print(f"[LinkedInAdapter] Apify search failed: {ae}. Falling back to Serper.")
        ...
        print(f"[LinkedInAdapter] Searching query via Serper fallback: {query}")
        try:
            serper_results = search_leads(query, tbs=timeframe, api_key=api_key, num=limit)
            return [r for r in serper_results if "linkedin.com" in (r.get("link") or "").lower()]
        except Exception as e:
            print(f"[LinkedInAdapter] Serper Fallback Error: {e}")
            return []
